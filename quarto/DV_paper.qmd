...
```{r}
#| fig-cap: Example estimates for total crimes using survey samples versus the reporting rate. The error ranges represent 95% binomial Clopper-Pearson exact intervals. Based on observed 500 crimes, a victimization rate of 2%, and a reporting rate of 65%.
#| fig-format: png
#| fig-width: 6.4
#| fig-height: 5
#| fig-dpi: 500
...
R <- 500
vic_rate <- 0.02 # 20 per 1000
rep_rate <- 0.65  # 70% reporting rate
tot <- R/rep_rate
tot_pop <- tot/vic_rate

# generating 100 to 10k surveys
surv_n <- seq(500,10000,by=20)

# This is Clopper-Pearson
# but can take non-integer arguments
clop <- function(x,n,a=0.05){
    lo <- qbeta(a/2,x,n-x+1)
    hi <- qbeta(1 - a/2,x+1,n-x)
    return(data.frame(cbind(lo,hi)))
}

# estimate for surveys
surv_err <- clop(surv_n*vic_rate,surv_n)*tot_pop
surv_err$type <- "Survey Estimate"
surv_err$sample <- surv_n

# estimate for reporting
rep_err <- clop(surv_n*vic_rate*rep_rate,surv_n*vic_rate)
tot_err <- R/rep_err # need to swith these around
tot_err2 <- tot_err
tot_err2[,1] <- tot_err[,2]
tot_err2[,2] <- tot_err[,1]
tot_err2$type <- "Reporting Estimate"
tot_err2$sample <- surv_n

# Using agresti-coull produces significantly smaller intervals
#ac <- binom.confint(surv_n*vic_rate*rep_rate,surv_n*vic_rate,methods=c("agresti-coull"))
#ac$rl <- rep_err[,1]
#ac$rh <- rep_err[,2]
#tot_err2[,2] <- R/ac$lower
#tot_err2[,1] <- R/ac$upper


df <- rbind(surv_err,tot_err2) #element_blank()

# Using agresti-coull produces significantly smaller intervals
ac <- binom.confint(surv_n*vic_rate*rep_rate,surv_n*vic_rate,methods=c("agresti-coull"))

p <- ggplot(data=df, alpha=0.5, aes(x=sample,fill=type,color=type,ymin=lo,ymax=hi)) + 
     geom_ribbon(linewidth=1.5,lineend="square") + scale_fill_manual(values = alpha(c("grey","white"), c(0.65,0.0))) +
     scale_color_manual(values=alpha(c("grey","black"), c(0.0,1.0))) +
     labs(x="Sample Size",title="Estimate of Total Crime",y="") +
     scale_x_continuous(breaks=seq(1000,10000,1000)) +
     theme_andy() + theme(legend.title = element_blank(), legend.position = c(0.8, 0.8),
...
```{r}
#| tbl-cap: Logistic regression results, predicting the probability of reporting to the police for 1,527 observations in the NCVS sample from 1992 through 2022.
#| tbl-cap-location: top

load(".././data/model.RData") # dva is the data, and mod is the model

mod_df <- coef(summary(mod))

orig_rows <- row.names(mod_df)
new_rows <- c( "(Intercept)"
              ,"rcs(year, c(1999, 2007, 2015))year"
              ,"rcs(year, c(1999, 2007, 2015))year'"
              ,"rcs(age, c(25, 40, 65))age"
              ,"rcs(age, c(25, 40, 65))age'"
              ,"female"
              ,"black"
              ,"nat"
...
row.names(mod_df) <- new_rows
kable(round(mod_df,2))
```
...
#| fig-height: 8
#| fig-width: 6
#| fig-cap: "Marginal Effect of Reporting Rate by Year and by Age"
#| fig-dpi: 500
...
ym <- ggplot(data=year_grade, aes(x=year,y=pred)) + 
      ...
      scale_y_continuous(breaks=seq(0.5,0.75,0.05), limits=c(0.5,0.75)) + 
...
```{r}
#| tbl-cap: Hosmer-Lemeshow calibration table,  $\chi^2 = 8.04$ p-value= 0.43
#| tbl-cap-location: top

dva$p0 <- 1*(dva$prep == 0)
dva$pObs <- 1 - dva$prob

fitdf <- aggregate(cbind(prep,prob,tot,p0,pObs) ~ cuts,data=dva,FUN=sum)
obs <- c(fitdf$prep,fitdf$p0)
exp <- c(fitdf$prob,fitdf$pObs)
chi <- sum((obs - exp)^2/exp)
p_val <- 1 - pchisq(chi,8)

fitdf[,3] <- round(fitdf[,3],1)
names(fitdf) <- c("Prob. Bin","Obs.","Exp.","Samp. Size","p0","pObs")
kable(fitdf[,1:4])
```
...
#| fig-height: 8
#| fig-width: 6
#| fig-cap: "Up-adjusted Total Aggravated Domestic Violence, Top 20 Cities in NIBRS 2022 data"
#| fig-dpi: 500
...
count_graph <- function(data,title=data$agency[1],height=5,width=10,res=1000) {
    # Added count graph
    ...
    tall$group <- factor(tall$group,c("Total","Reported"))
    p <- ggplot(tall,aes(x=year,y=tot,fill=group)) +
         geom_bar(position="stack",stat="identity",color="black") +
         scale_fill_manual(values = c("#018571","#dfc27d"), name="") + 
...
rate_graph <- function(data,title=data$agency[1],height=5,width=8,res=1000) {
    p <- ggplot(data,aes(x=year,y=WeightRate,ymin=LowRate,ymax=HighRate)) +
         geom_ribbon(fill="#018571",alpha=0.7) + 
         expand_limits(y=0) +
...
# Estimate of up/down
big <- up2022[up2022$tot > 50,]
# summary(big$weight/big$tot)  # 23% to 77%
...
#     geom_point(aes(x=weight,y=agency,fill="Total"),pch=21,color="black",size=5.5) +

p <- ggplot(up2022) +
     geom_bar(aes(x=weight,y=agency,fill="Total"),stat="identity",color="black") +
     geom_bar(aes(x=tot,y=agency,fill="Reported"),stat="identity",color="black") +
     scale_fill_manual(values = c("#dfc27d","#018571"), name="") +
     labs(x='Domestic Violence Counts',y="") +
     theme_andy() +
     theme(axis.text.x = element_text(size=12)) + theme(legend.title = element_blank(), legend.position = c(0.8, 0.2),
     legend.spacing.y = unit(0, "mm"),
     legend.background = element_rect(fill="white",
                                  linewidth=0.5, linetype="solid", 
                                  colour ="black"))
...
```{r}
#| fig-cap: "Estimates of Total Domestic Aggravated Assaults in Denver"
#| fig-dpi: 500
#| fig-width: 6

denver <- prep_ori("CODPD0000")
p <- count_graph(denver,"Denver")
p
```
...
```{r}
#| fig-cap: "Estimates of Total Domestic Aggravated Assault Rate with Simulated Confidence Intervals in Honolulu"
#| fig-dpi: 500

honolulu <- prep_ori("HI0020000")
p <- rate_graph(honolulu,"Honolulu")
p
```
...
```{r}
#| fig-cap: "Estimates of Total Domestic Aggravated Assault Rates for Selected Jurisdictions"
#| fig-dpi: 500
#| fig-height: 6
#| fig-width: 6
...
wichita <- prep_ori("KS0870300")

# Making a small multiple graph
stacked <- rbind(memphis,detroit,saltlake,wichita)

stacked$agency <- gsub("POLICE DEPARTMENT","",stacked$agency)
stacked$agency <- gsub("PD","",stacked$agency)
stacked$agency <- gsub("POLICE ADMIN","",stacked$agency)
stacked$agency <- gsub("METROPOLITAN","",stacked$agency)
stacked$agency <- str_squish(stacked$agency)

p <- ggplot(stacked,aes(x=year,y=WeightRate,ymin=LowRate,ymax=HighRate)) +
     geom_line(color="black",linewidth=0.8) + 
     geom_point(fill='black',color='white',pch=21,size=2) +
     facet_wrap(~agency) +
